...
def get_base_hand_env(
    ...
    class BaseHandEnv(RobotEnvClass):
        ...
        def __init__(self, relative_control, **kwargs):
            ...
            # 自动适配 action_space 维度
            if hasattr(self, 'model') and hasattr(self.model, 'nu'):
                self.action_space = spaces.Box(low=-1., high=1., shape=(self.model.nu,), dtype=np.float32)
            elif hasattr(self, 'sim') and hasattr(self.sim.model, 'nu'):
                self.action_space = spaces.Box(low=-1., high=1., shape=(self.sim.model.nu,), dtype=np.float32)
...
class MujocoPyHandEnv(get_base_hand_env(MujocoPyRobotEnv)):
    ...
    def _set_action(self, action):
        super()._set_action(action)
        assert action.shape == (self.sim.model.nu,)
    ...
    def __init__(self, default_camera_config: dict = DEFAULT_CAMERA_CONFIG, **kwargs) -> None:
        super().__init__(default_camera_config=default_camera_config, **kwargs)
        # 再次确保 action_space 维度正确
        self.action_space = spaces.Box(low=-1., high=1., shape=(self.sim.model.nu,), dtype=np.float32)
