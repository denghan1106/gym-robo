...
class MujocoManipulateTouchSensorsEnv(MujocoManipulateEnv):
    ...
    def reset_hand_and_object(self):
        # 获取object初始位置
        object_qpos = self._utils.get_joint_qpos(self.model, self.data, "object:joint")
        object_pos = object_qpos[:3]
        # hand_base对齐object的x/y，z高于object
        hand_x = object_pos[0]
        hand_y = object_pos[1]
        hand_z = 0.5  # 可根据需要调整

        # 兼容新版 mujoco，直接操作 qpos
        import mujoco
        for joint_name, value in [("arm:x", hand_x), ("arm:y", hand_y), ("arm:z", hand_z), ("arm:rotz", 0.0)]:
...
class MujocoPyManipulateTouchSensorsEnv(MujocoPyManipulateEnv):
    ...
    def reset_hand_and_object(self):
        ...
        import mujoco
        for joint_name, value in [("arm:x", hand_x), ("arm:y", hand_y), ("arm:z", hand_z), ("arm:rotz", 0.0)]:
            joint_id = mujoco.mj_name2id(self.sim.model, mujoco.mjtObj.mjOBJ_JOINT, joint_name)
            if joint_id != -1:
